...
- 修复右键菜单底部项被顶出可视区：最大高度改为按"菜单顶边到可见区底部"计算，并在子菜单展开后重算，超出即出滚动条、展开内容自动滚入视野
- 正在播放识别优先选择"正在播放"的会话，多播放器并存时不会认错；QQ音乐/酷狗同样走该通用路径（需在播放器设置中开启系统媒体控件）
- 歌词解析过滤 QQ/酷狗的"词：/曲："制作信息行与"歌名 - 歌手"标题行，并解码 HTML 实体
