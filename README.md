...
mac 版目前**没有签名/公证**（省掉 Apple 开发者账号 $99/年），所以首次打开会被 Gatekeeper 拦下，属正常现象。
会被拦下时提示词有两种，对应两种处理方式：

**① 提示「Apple 无法验证"Petra"是否包含可能危害 Mac 安全或泄漏隐私的恶意软件」**

系统设置 → 隐私与安全性 → **滑到最底部** → 点「仍要打开」→ 用指纹或密码确认（只需做一次）。

**② 提示「"Petra.app"已损坏，无法打开。您应该将它移到废纸篓」**

这一条是解压后文件带上了「隔离」标记（不是文件真的坏了）。打开「终端」，把下面命令里的路径换成你实际的 app 路径后执行：

```bash
sudo xattr -rd com.apple.quarantine /Applications/Petra.app
```

（输入密码时终端不显示任何字符，属正常；执行完再双击就能打开）

> 首次放行之后，走**应用内的自动更新**一般不会再出现这两种提示（更新是应用自己写文件，不会带隔离标记）。
> 想让用户完全零操作，需要 Apple 开发者账号做签名 + 公证 —— 等 mac 用户量上来再考虑。

mac 版与 Windows 版的功能差异（其余功能一致）：

| 功能 | macOS 状态 |
|---|---|
| PSD / Live2D 模型、待机、摸头、桌面漫游、小助手、日记、抽卡、整点播报 | ✅ 可用 |
| 打开软件 / 打开文件 / 锁屏 / 调音量 / 系统通知 | ✅ 可用（锁屏用系统自带的 CGSession，**不需要辅助功能权限**） |
| 在线歌词 + 歌词气泡 | ✅ 可用（AppleScript 读 **Apple Music / Spotify** 的曲目与进度，歌词取自 LRCLIB） |
| 歌词翻译 | ➖ 不提供（mac 侧数据源没有译文，菜单里不会出现这一项） |
| 读取"当前在用什么软件" | ✅ 可用（NSWorkspace，**不需要权限**；只给应用名，不给窗口标题） |
| 天气 | ✅ 可用（curl 取 wttr.in；按 IP 或你填的城市定位，不申请定位权限） |
| 定时关机 | ✅ 可用（应用内定时器 + AppleScript；**需要 Petra 保持运行**） |
| 跟随音乐（按节拍跟唱、声音驱动的摇摆） | ❌ 不提供（Windows 靠 WASAPI 系统回环录音；mac 需要装 BlackHole 虚拟声卡或调 CoreAudio 私有接口） |

mac 上有两处会弹系统权限，都是**用到才弹、拒绝了也不影响其它功能**：

- **自动化**：第一次读取 Apple Music / Spotify 的曲目时弹出（供歌词气泡使用）
- **通知**：第一次发系统通知时弹出
