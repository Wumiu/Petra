...
H --> J["平台产物<br/>Windows/macOS/Linux"]
```
...
## 架构总览
下图展示了从源码到可执行包的端到端构建流程，包括前端构建、Rust编译、Tauri打包与各平台产物输出。
...
Win --> |是| WinPack["配置签名与依赖<br/>生成MSI/EXE"]
Win --> |否| Mac{"macOS?"}
Mac --> |是| MacPack["代码签名与公证<br/>生成.app/.dmg"]
Mac --> |否| LinPack["生成AppImage/deb/rpm"]
...
## 结论
本指南提供了从前端到后端的完整构建与部署方案，覆盖Vite与TypeScript配置、Rust后端构建、多平台打包、签名与发布、CI/CD集成以及性能优化。遵循本文流程可确保构建稳定性、安全性与可维护性，同时兼顾性能与包体积。
