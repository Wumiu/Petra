...
## 更新摘要
**所做更改**
- 更新了环境要求部分，增加了Node.js版本建议和Rust工具链安装说明
- 增强了依赖安装步骤，提供了更详细的跨平台安装指导
- 新增了自定义角色创建功能的开发环境配置说明
- 完善了项目结构概览，突出了Live2D和助手功能模块
- 更新了故障排除指南，包含更多常见问题的解决方案
...
**新增功能**：本项目现已支持自定义角色创建功能，开发者可以通过配置文件和脚本轻松添加新的桌宠角色，包括Live2D模型集成、行为引擎配置和UI界面定制。
...
### 环境与系统要求
- Node.js
  - 版本建议：推荐使用 Node.js 18.x LTS 或更高版本，确保与 Tauri 和 Vite 的最佳兼容性
  - 验证方式：在终端执行 `node --version` 和 `npm --version`，确认已安装且可用
- Rust 工具链
...
**更新**：新增了Node.js版本建议和具体的验证命令，便于开发者快速检查环境配置。
...
- 安装 Node.js 依赖
  ...
  - 建议使用 `npm install` 或 `pnpm install` 以获得最佳性能
- 安装 Rust 与 Tauri 系统依赖
  - 使用 rustup 安装最新稳定版工具链：`curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh`
  - 根据平台安装 Tauri 所需的系统库（例如 macOS 的 `xcode-select --install`，Linux 的 webkitgtk 开发包）
- 验证安装
  - 运行开发服务器与构建命令，确认无报错
  - 执行 `cargo build` 验证 Rust 环境配置正确

**更新**：增加了具体的安装命令和验证步骤，提供更详细的安装指导。
...
**更新**：增加了通用的启动流程和平台差异说明，便于不同平台的开发者快速上手。
...
- src：前端源码
  ...
  - assistant/：助手功能模块，处理AI对话和交互
  - live2d/：Live2D角色渲染和控制
  - autonomous/：自主行为引擎，定义角色智能行为
  - audio/：音频分析和处理
  - ui/：用户界面组件
  - utils/：工具类和辅助函数
...
**更新**：详细说明了各个功能模块的职责，特别是新增的助手功能和Live2D模块。
...
### 自定义角色创建指南
**新增功能**：项目现已支持灵活的自定义角色创建系统，开发者可以轻松添加新的桌宠角色。

- 角色配置文件结构
  - 在 `public/models/` 目录下创建角色配置文件
  - 支持Live2D模型、行为脚本和UI配置的分离管理
- 创建新角色的步骤
  1. 准备Live2D模型文件和配置文件
  2. 在manifest.json中添加角色元数据
  3. 配置角色行为和交互逻辑
  4. 测试角色在应用中的显示效果
- 角色行为定制
  - 使用autonomous模块定义角色的自主行为
  - 通过assistant模块集成AI对话功能
  - 自定义音频反馈和视觉特效
...
**更新**：新增了自定义角色相关的故障排除指南，帮助开发者解决角色创建过程中的常见问题。
...
## 结论
通过以上步骤，你可以在 Windows、macOS 与 Linux 平台上快速搭建开发环境并运行桌面宠物应用。现在项目还支持灵活的自定义角色创建功能，让开发者能够轻松扩展应用的个性化体验。建议在熟悉基础流程后，进一步阅读各模块源码以理解前后端交互与系统能力集成方式。遇到问题时，优先检查环境版本、依赖安装与权限配置。

**新增**：自定义角色功能为应用带来了更大的可扩展性，开发者可以根据需求创建独特的桌宠角色，提升用户体验。
...
**更新**：新增了角色开发相关的工具和命令，为自定义角色开发提供支持。
